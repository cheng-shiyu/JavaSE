 ...
 * 增加缩容操作
 */
public class ArrayList1<E> extends AbstractList<E> implements List<E>{

    // 存放元素的数组
    private E[] elements;

    // 初始容量  默认是16个长度
    private static final int DEFAULT_CAPATICY = 8;
    ...
    public void trim(){
        ...
        /*目标，如果没有使用够一半，那么就进行缩容操作，但是最低缩容到8就停止缩容*/
        if (size > ncapacity || capacity <= DEFAULT_CAPATICY){
            return;
        }
        /*进行缩容操作*/
        // 创建一个空数组
        E[] nelements = (E[]) new Object[ncapacity];
        for (int i = 0; i < size; i++){
            nelements[i] = elements[i];
        }
        System.out.println("缩容之后的剩余长度 = " + ncapacity);
        elements = nelements;
    }
}
