package com.chengshiyu.Array;

/**
 * @author 程世玉
 * @data 2022/4/22.
 * 双向链表
 * 虚拟头节点暂时没有设置
 */
public class twoLinkedList<E> extends AbstractList<E> implements List<E> {

    private Node<E> first;
    private Node<E> last;

    public twoLinkedList() {

    }


    /*这个是用户添加对象的时候*/
    private class Node<E> {
        Node<E> prev;
        E element;
        Node<E> next;

        public Node<E> getPrev() {
            return prev;
        }

        public void setPrev(Node<E> prev) {
            this.prev = prev;
        }

        public E getElement() {
            return element;
        }

        public void setElement(E element) {
            this.element = element;
        }

        public Node<E> getNext() {
            return next;
        }

        public void setNext(Node<E> next) {
            this.next = next;
        }

        public Node(Node<E> prev, E element, Node<E> next) {
            this.prev = prev;
            this.element = element;
            this.next = next;
        }
    }

    @Override
    public void clear() {
        first = null;
        last = null;
    }

    @Override
    public boolean contains(E a) {
        return false;
    }

    @Override
    public void add(E a) {
        add(size,a);
//        /*从0开始进行插入,则需要配置first，因为first开始默认是null*/
//        /*获取前一个元素*/
//        Node<E> prev = getIndexNode(size - 1);
//        /*让他指向他，接着之前的再指向之前的*/
//        prev.next = new Node<>(a, prev.next);
//        size++;
    }

    @Override
    public E get(int a) {
        Node<E> node = getIndexNode(a);
        return node.getElement();
    }

    @Override
    public Object set(int index, E a) {
        return null;
    }

    /**
     * 新添加的节点
     */
    @Override
    public void add(int index, E a) {

      if (index == size){
          /*
          1、第一个元素，first和last都是null，一定经过这里
          2、那么每次进行添加之后，last应该怎么更新
           */
          // 获取到当前谁在最后一个oldlast
          Node<E> oldlast = this.last;
          // 创建到目前最后的元素信息
           last = new Node<>(oldlast, a, null);
          if (oldlast == null){
              first = last;
          }else {
              oldlast.next = last;
          }
      }else {
          // 获取当前元素，当前元素必定是要被添加元素的next
          Node<E> next = getIndexNode(index);
          // 获取当前元素的头元素，这个元素必定是要添加元素的prev
          Node<E> prev = next.prev;
          // 当前元素如下：
          Node<E> node = new Node<>(null, a, next);

          // 当前元素的前一个元素变成添加的元素
          next.prev = node;

          // 进行判断prev到底是什么情况，如果是null，那么就直接让first等于node
          if (prev == null){
              first = node;
          }else {
              prev.next = node;
          }
      }

      size++;


        // 自己的思路：！！！！
////        /*获取到要插入位置原本的节点信息*/
//        Node<E> indexNode = getIndexNode(index);
//        Node<E> prev = indexNode.prev;
//        /*创建新添加的节点的对象*/
//        Node<E> addNode = new Node<>(prev, a, indexNode);
//
//        /*考虑特殊情况，如果添加到0的位置怎么办，如果添加到最后的位置怎么办*/
//        if (prev == null){
//            /*创建新添加的节点的对象*/
//            first = addNode;
//        }else {
//            prev.next = addNode;
//            indexNode.prev = addNode;
//        }
    }

    /**
     * 删除指定元素
     */
    @Override
    public void remove(int index) {
        Node<E> indexNode = getIndexNode(index);

        Node<E> next = indexNode.next;
        Node<E> prev = indexNode.prev;

        // 把中间这个元素给跳过去，让上一个执行下下一个，让下下一个指向上上一个
//        prev.next = next;
//        next.prev = prev;

        // 需要考虑头节点还有尾节点的情况,之所以出现这种情况，还是因为first仅仅就是一个变量，所以头节点的prev是null，而不是指向first
        if (prev == null){
            first.next = next;
        }else {
            prev.next = next;
        }

        if (next == null){
            last.next = prev;
        }else {
            next.prev = prev;
        }
        size--;


    }

    @Override
    public int indexOf(Object element) {
        return 0;
    }

    /**
     * 获取根据index获取指定节点信息
     */
    private Node<E> getIndexNode(int index) {
        /*如果小于一半，则从开头进行查询*/
        if (index < (size>>1)){
            Node<E> node = first;
            for (int i = 0; i < index; i++) {
                node = node.next;
            }
            return node;
        }else {
            Node<E> node = this.last;
            for (int i = size-1; i > index;i--){
                node = node.prev;
            }
            return node;
        }

    }
}
